...
var (
	...
	DefaultMigrationsTable       = "_calliope"
...
func (p *Postgres) Open(url string) (database.Driver, error) {
	...
	if (len(migrationsTable) > 0) && (migrationsTableQuoted) && ((migrationsTable[0] != '"') || (migrationsTable[len(migrationsTable)-1] != '"')) {
		return nil, fmt.Errorf("x-migrations-table must be quoted (for instance '\"migrate\".\"_calliope\"') when x-migrations-table-quoted is enabled, current value is: %s", migrationsTable)
	}
